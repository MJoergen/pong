...
architecture structural of pong is
  ...
  constant C_PLAYER_BITMAP : pattern_t := (
                                            others => "111111100000000000000000");
  constant C_BALL_BITMAP   : pattern_t := (
...
begin
  ...
  -- Instantiate Player
  player_inst : entity work.player_move
    port map (
      clk_vs_i   => vga_vs,
      btn_up_i   => btn_i(3),
      btn_down_i => btn_i(0),
  ...
  sprites_inst : entity work.sprite
    port map (
      clk_i       => clk_i,
      vga_vs_i    => vga_vs,
      sprites_i   => sprites,
  ...
  vga_col_o          <= vga_col when blank = '0' else
                        "00000000";
